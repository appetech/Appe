...
class AppePreparedReport(Document):
	def after_insert(self):
		if self.appe_api_integration:
			try:
				report_name = self.third_party_report_name
				...
				url = frappe.db.get_value("Appe API Integration",self.appe_api_integration,"url")
				url = url+"/api/method/frappe.desk.query_report.run"

				headers = {
					"Authorization": frappe.db.get_value("Appe API Integration",self.appe_api_integration,"token"),
				...
				frappe.log_error("Appe Prepared Report response api",str(response.text))

				if response.status_code == 200:
					data = response.json()

					report_data = data.get("message", {})

					self.status = "Completed"
					self.finished_at = now_datetime()
					self.results = json.dumps(report_data)
				...
				frappe.log_error("Appe Prepared Report: general failure",str(e))
