(() => {
  // ../appe/appe/public/js/appe_buddy_panel.bundle.js
  ...
  (function() {
    ...
    function init() {
      ...
      $(document).on(
        "appe-buddy-settings-saved",
        () => fetch_settings_and_toggle({ force: true })
      );
      if (window.frappe && frappe.realtime && frappe.realtime.on) {
        frappe.realtime.on(
          "appe_buddy_settings_changed",
          () => fetch_settings_and_toggle({ force: true })
        );
      }
    ...
    function fetch_settings_and_toggle({ force = false, throttle = false } = {}) {
      const now = Date.now();
      if (throttle && !force && now - _last_check_at < 5e3)
        return;
    ...
    appe.buddy.FloatingPanel = class FloatingPanel {
      ...
      summarize_tool_result(m) {
        ...
        if (typeof r === "object") {
          ...
          return frappe.utils.escape_html(preview.join(" \xB7 ") || "ok");
        }
        return frappe.utils.escape_html(String(r).slice(0, 140));
      }
      format_text(text) {
        const escaped = frappe.utils.escape_html(text);
        return escaped.replace(/```([\s\S]*?)```/g, (_, code) => `<pre>${code}</pre>`).replace(/`([^`]+)`/g, "<code>$1</code>").replace(/\n/g, "<br>");
      }
      scroll_bottom() {
      ...
      update_context_chip() {
        ...
        if (ctx.doctype && ctx.docname)
          label = `${ctx.doctype} \xB7 ${ctx.docname}`;
        else if (ctx.doctype)
          label = ctx.doctype;
        else if (ctx.report)
          label = `Report \xB7 ${ctx.report}`;
        else if (ctx.dashboard)
          label = `Dashboard \xB7 ${ctx.dashboard}`;
      ...
      call(method, args = {}) {
        const isPost = method.indexOf("send_") !== -1 || method.indexOf("new_") !== -1 || method.indexOf("rename_") !== -1 || method.indexOf("pin_") !== -1 || method.indexOf("archive_") !== -1 || method.indexOf("delete_") !== -1;
        return new Promise((resolve, reject) => {
          frappe.call({
            ...
            callback: (r) => {
              ...
              } else if (r.message && r.message.data !== void 0) {
...
//# sourceMappingURL=appe_buddy_panel.bundle.ZF7OZZR5.js.map
